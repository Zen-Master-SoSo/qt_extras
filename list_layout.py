...
class GListLayout(QGridLayout, _ListLayout):
	...
	def set_columns(self, columns):
		if self.flow != GListLayout.HORIZONTAL_FLOW:
			raise Warning('Setting columns has no effect when flow is horizontal')
		if columns != self.columns_or_rows:
	...
	def set_rows(self, rows):
		if self.flow != GListLayout.HORIZONTAL_FLOW:
			raise Warning('Setting rows has no effect when flow is vertical')
		if rows != self.columns_or_rows:
			self._take_all_from(0)
